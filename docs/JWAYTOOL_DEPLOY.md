...
`jwaytool.com` 도메인에 JWAY Paperclip을 올리는 **가장 쉬운 방법**입니다.
...
```
jwaytool.com/          ← 랜딩페이지 (index.html)
jwaytool.com/app.html  ← 실제 앱 (원래 index.html이었던 것)
jwaytool.com/templates/ ← 팀 템플릿 JSON 파일들
jwaytool.com/examples/ ← 결과물 샘플
```

## 방법 1: GitHub Pages + jwaytool.com (무료, 추천)
...
2. **Custom domain**: `jwaytool.com` 입력 → Save
3. 체크 **Enforce HTTPS** (5~30분 후 체크박스 활성화됨)

### Step 4: 확인

- DNS 전파 대기 5분 ~ 최대 24시간
- `https://jwaytool.com` 접속 → 랜딩페이지 표시
- `https://jwaytool.com/app.html` 접속 → 앱 표시
...
- `http://jwaytool.com` 접속 확인

## 배포 후 체크리스트

- [ ] `jwaytool.com`에서 랜딩페이지가 뜨는가?
- [ ] "무료로 시작" 버튼 클릭 시 `app.html`로 이동하는가?
- [ ] `app.html`에서 앱이 정상 로드되는가? (비밀번호 설정 화면)
- [ ] `templates/` 폴더가 접근 가능한가? (`jwaytool.com/templates/`)
...
### "사이트를 찾을 수 없음" 에러
- DNS 전파 대기 (최대 24시간)
- `nslookup jwaytool.com`으로 IP 확인
...
### app.html이 안 열림
- 파일 경로 확인: `jwaytool.com/app.html` 직접 접속
